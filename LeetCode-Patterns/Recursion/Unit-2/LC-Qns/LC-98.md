...
            System.out.println(isBST + " "  + min + " " + max);
...
        if(root == null) return new ret();

        //Divide

        ret leftRes=helper(root.left);

        ret rightRes=helper(root.right);

  

        if(leftRes.isBST == false || rightRes.isBST == false) return new ret(false);
...
        Ok so leftRes.max < val < rightRes.min global max and min

        */

        if(!(leftRes.max < root.val && root.val < rightRes.min)) return new ret(false);

  

        long min = Math.min((long)root.val, leftRes.min);

        long max = Math.max((long)root.val, rightRes.max);
...
🌱 **Learning Summary — Validate Binary Search Tree (Divide & Conquer, Bottom-Up)**
...
- “If the left child is smaller and the right child is bigger, it’s a BST.”
    
- Or slightly improved: “If both subtrees are BSTs and `left.val < root < right.val`, we’re good.”
...
- Tree traversal examples reinforce parent–child comparisons.
...
- A node deep in the left subtree can violate the root’s constraint without violating its immediate parent.
...
> “Is this node correct relative to its children?”

Shift to:

> “What **range of values** is this subtree allowed to occupy?”
...
Every subtree is no longer just “valid or invalid” —  
...
Once seen this way, recursion isn’t a trick — it’s a necessity.

---

### 🟣 3️⃣ Design Decisions and Their Necessity

Each major design choice you made was not arbitrary — it was forced by the problem.
...
- A boolean answers _“is this subtree valid?”_
    
- It does **not** answer _“what values live here?”_
...
Your approach chose reporting — which naturally leads to:
...
That wasn’t stylistic.  
It was a direct consequence of information flow.

---

### 🟡 4️⃣ Reasoning Artifacts — Why Your Artifacts Matter

Your comments like:

> “Ok so leftRes.max < val < rightRes.min”

seem obvious _after_ the fact — but they prevent a very specific confusion:
...
- The mind defaults to what’s directly visible.
...
Rather than re-explaining the code, here’s why the **non-obvious parts** exist.
...
- **Returning sentinel values for null nodes**
    ...
    - Prevents branches like “if left exists…”
...
- Those constants feel “extreme enough”
...
The structure isn’t about elegance — it’s about _containment of thought_.
...
**Constraints that looked important but weren’t**
...
**Pattern Name**  
👉 **Bottom-Up Subtree Aggregation with Invariants**
...
> _When the problem feels global but the checks feel local — stop checking nodes.  
...
The errors weren’t mistakes — they were the edges of your mental model being discovered.

That’s exactly how real understanding forms.